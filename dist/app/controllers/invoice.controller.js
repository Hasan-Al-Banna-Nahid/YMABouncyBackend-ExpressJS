...
exports.generateCustomInvoiceHandler =
  exports.generateInvoiceForBookingHandler =
  exports.deleteInvoiceHandler =
  exports.updateInvoiceHandler =
  exports.getInvoicesByBookingHandler =
  exports.getInvoicesHandler =
...
function ownerId(user) {
  return (
    user instanceof mongoose_1.Types.ObjectId ? user : user._id
  ).toString();
}
/** POST /invoices */
const createInvoiceHandler = async (req, res, next) => {
  try {
    const aReq = req;
    if (!aReq.user) new throw ApiError_1.default("User not found", 404);
    const invoiceData = {
      ...aReq.body,
      user: aReq.user.id, // string OK; Mongoose will cast
    };
    const invoice = await (0, invoice_service_1.createInvoice)(invoiceData);
    (0, apiResponse_1.ApiResponse)(res, 201, "Invoice created successfully", {
      invoice,
    });
  } catch (err) {
    next(err);
...
const getInvoiceHandler = async (req, res, next) => {
  try {
    ...
    (0, apiResponse_1.ApiResponse)(res, 200, "Invoice retrieved successfully", {
      invoice,
    });
  } catch (err) {
    next(err);
...
const getInvoicesByBookingHandler = async (req, res, next) => {
  try {
    const aReq = req;
    if (!aReq.user) new throw ApiError_1.default("User not found", 404);
    const invoices = await (0, invoice_service_1.getInvoicesByBooking)(
...
const updateInvoiceHandler = async (req, res, next) => {
  try {
    ...
    const updatedInvoice = await (0, invoice_service_1.updateInvoice)(
      req.params.id,
      req.body
    );
...
const generateInvoiceForBookingHandler = async (req, res, next) => {
  try {
    ...
    const invoice = await (0, invoice_service_1.generateInvoiceForBooking)(
      req.params.bookingId,
      isOrganization
    );
    (0, apiResponse_1.ApiResponse)(res, 201, "Invoice generated successfully", {
      invoice,
    });
  } catch (err) {
    next(err);
...
const generateCustomInvoiceHandler = async (req, res, next) => {
  try {
    ...
    const invoice = await (0, invoice_service_1.generateCustomInvoice)(
      aReq.user.id,
      amount,
      description,
      isOrganization
    ...
    (0, apiResponse_1.ApiResponse)(
      res,
      201,
      "Custom invoice generated successfully",
      { invoice }
    );
  } catch (err) {
    next(err);
