...
exports.releaseExpiredCartItems =
  exports.checkInventoryAvailability =
  exports.getBookedInventory =
  exports.getAvailableInventory =
  exports.deleteInventoryItem =
  exports.updateInventoryItem =
  exports.getInventoryItems =
...
const toObjectId = (val) => {
  if (val instanceof mongoose_1.Types.ObjectId) return val;
  if (val && val._id instanceof mongoose_1.Types.ObjectId) return val._id;
...
const getInventoryItem = async (id) => {
  ...
  if (!inventoryItem) {
    new throw ApiError_1.default("No inventory item found with that ID", 404);
  }
  return inventoryItem;
};
...
const updateInventoryItem = async (id, updateData) => {
  const inventoryItem = await inventory_model_1.default.findByIdAndUpdate(
    id,
    updateData,
    {
      new: true,
...
const releaseExpiredCartItems = async () => {
  ...
  const expiredBookings = await booking_model_1.default
    .find({
      status: "pending",
