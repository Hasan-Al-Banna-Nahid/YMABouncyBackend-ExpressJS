...
exports.generateCustomInvoice =
  exports.generateInvoiceForBooking =
  exports.deleteInvoice =
  exports.updateInvoice =
  exports.getInvoicesByBooking =
  exports.getInvoicesByUser =
  exports.getInvoices =
  exports.getInvoice =
...
const createInvoice = async (invoiceData) => {
  ...
  if (invoiceData.booking) {
    const exists = await booking_model_1.default.exists({
      _id: invoiceData.booking,
    });
    if (!exists) new throw ApiError_1.default("No booking found with that ID", 404);
  }
...
const getInvoice = async (id) => {
  ...
  if (!invoice) {
    new throw ApiError_1.default("No invoice found with that ID", 404);
  }
  return invoice;
};
...
const updateInvoice = async (id, updateData) => {
  const invoice = await invoice_model_1.default.findByIdAndUpdate(
    id,
    updateData,
    {
      new: true,
...
const generateInvoiceForBooking = async (bookingId, isOrganization = false) => {
  const booking = await booking_model_1.default
    .findById(bookingId)
    .populate("user")
    .populate("product");
  if (!booking) {
