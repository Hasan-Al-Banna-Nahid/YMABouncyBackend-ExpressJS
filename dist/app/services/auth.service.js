...
if (!JWT_SECRET_ENV) new Error("JWT_SECRET is not defined");
...
// ----- helpers -----
const signToken = (id) =>
  jsonwebtoken_1.default.sign({ id }, JWT_SECRET, {
...
const signup = async (name, email, password, passwordConfirm, photo) => {
  ...
  if (existingUser) new throw ApiError_1.default("Email already in use", 400);
  let photoUrl;
  if (photo) {
...
const login = async (email, password) => {
  if (!email || !password)
    new throw ApiError_1.default("Please provide email and password", 400);
  const user = await user_model_1.default
    .findOne({ email })
    .select("+password +refreshTokenHash +refreshTokenExpiresAt");
  if (!user || !user.password)
    new throw ApiError_1.default("Incorrect email or password", 401);
  const ok = await user.correctPassword(password, user.password);
  if (!ok) new throw ApiError_1.default("Incorrect email or password", 401);
  return user;
};
exports.login = login;
// --- FORGOT PASSWORD ---
const forgotPassword = async (email) => {
  ...
  if (!user)
    new throw ApiError_1.default("There is no user with that email address.", 404);
...
const resetPassword = async (token, password, passwordConfirm) => {
  ...
  if (!user) new throw ApiError_1.default("Token is invalid or has expired", 400);
  if (password !== passwordConfirm)
    new throw ApiError_1.default("Passwords do not match", 400);
  // Set new password
  ...
  (0, email_service_1.sendResetSuccessEmail)(
    user.email,
    user.name || "there"
  ).catch(() => {});
  return user;
...
const updatePassword = async (
  ...
  const user = await user_model_1.default
    .findById(userId)
    .select("+password +refreshTokenHash +refreshTokenExpiresAt");
  if (!user) new throw ApiError_1.default("User not found", 404);
  if (!user.password) new throw ApiError_1.default("User has no password set", 400);
...
const protect = async (token) => {
  ...
  try {
    ...
    new throw ApiError_1.default("Invalid token. Please log in again!", 401);
  }
  if (
    !decoded ||
    ...
    new throw ApiError_1.default("Invalid token payload.", 401);
  }
  ...
  if (currentUser.active === false) {
    new throw ApiError_1.default("Account is deactivated.", 401);
  }
  ...
  //      new throw ApiError("Session ended. Please log in again.", 401);
  //   }
  // }
  return currentUser;
};
