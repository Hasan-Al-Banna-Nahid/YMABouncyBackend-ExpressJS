...
export const createInventoryItem = async (inventoryData: IInventory) => {
    ...
    if (existingItem) {
        throw new ApiError('Inventory item already exists for this product and date', 400);
    }
...
export const updateInventoryItem = async (id: string, updateData: Partial<IInventory>) => {
...
export const getAvailableInventory = async (productId: string, startDate: Date, endDate: Date) => {
...
export const getBookedInventory = async (productId: string, startDate: Date, endDate: Date) => {
    return await Inventory.find({
        product: productId,
        date: { $gte: startDate, $lte: endDate },
        status: 'booked',
...
export const checkInventoryAvailability = async (productId: string, date: Date) => {
