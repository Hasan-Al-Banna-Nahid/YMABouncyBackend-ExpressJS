...
type CreateInvoiceInput = Omit<IInvoice, 'invoiceNumber' | 'user' | 'booking'> & {
...
export const updateInvoice = async (id: string, updateData: Partial<IInvoice>) => {
...
export const generateInvoiceForBooking = async (bookingId: string, isOrganization = false) => {
    const booking = await Booking.findById(bookingId).populate('user').populate('product');
    if (!booking) {
