...
export const getInvoiceHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (ownerId(invoice.user as any) !== aReq.user.id && aReq.user.role !== 'admin') {
...
export const getInvoicesHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const invoices =
            aReq.user.role === 'admin' ? await getInvoices() : await getInvoicesByUser(aReq.user.id);
...
export const getInvoicesByBookingHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const aReq = req as AuthenticatedRequest;
        if (!aReq.user) throw new ApiError('User not found', 404);
...
export const updateInvoiceHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        ApiResponse(res, 200, 'Invoice updated successfully', { invoice: updatedInvoice });
    } catch (err) {
        next(err);
    }
};
...
export const generateInvoiceForBookingHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { isOrganization } = req.body;
        const invoice = await generateInvoiceForBooking(req.params.bookingId, isOrganization);
...
export const generateCustomInvoiceHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const aReq = req as AuthenticatedRequest;
        if (!aReq.user) throw new ApiError('User not found', 404);

        const { amount, description, isOrganization } = aReq.body;
        const invoice = await generateCustomInvoice(aReq.user.id, amount, description, isOrganization);
